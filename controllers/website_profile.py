...
class CustomWebsiteProfile(WebsiteProfile):
    ...
    def view_user_profile_edition(self, **kwargs):
        ...
        ils = request.env['bas.il'].sudo().search([])
        unis = request.env['bas.uni'].sudo().search([])
        ...
        ilces = request.env['bas.ilce'].sudo().search([('il_id', '=', user.il_id.id)]) if user.il_id else []
        faks = request.env['bas.fak'].sudo().search([('uni_id', '=', user.uni_id.id)]) if user.uni_id else []
        liseler = request.env['bas.lise'].sudo().search([('ilce_id', '=', user.ilce_id.id)]) if user.ilce_id else []
        bolumler = request.env['bas.bolum'].sudo().search([('fak_id', '=', user.fak_id.id)]) if user.fak_id else []
    ...
    def get_ilce(self, il_id):
        ilceler = request.env['bas.ilce'].sudo().search([('il_id', '=', int(il_id))])
    ...
    def get_lise(self, ilce_id):
        liseler = request.env['bas.lise'].sudo().search([('ilce_id', '=', int(ilce_id))])
    ...
    def get_fak(self, uni_id):
        fakulteler = request.env['bas.fak'].sudo().search([('uni_id', '=', int(uni_id))])
    ...
    def get_bolum(self, fak_id):
        bolumler = request.env['bas.bolum'].sudo().search([('fak_id', '=', int(fak_id))])
